...
export class ActionPipeline {
    ...
    public constructor(public readonly actor: Actor) {
        this.movementSubscription =
            this.actor.walkingQueue.movementQueued$.subscribe(async () =>
    ...
    public static register(
        action: ActionType,
        actionPipeHandlerFn: ActionPipeHandler,
    ): void {
    ...
    private async runActionHandler(
        actionHandler: any,
        args: any[],
    ): Promise<void> {
        const runnableHooks: RunnableHooks | null | undefined =
    ...
    private async runHook(actionHook: ActionHook, action: any): Promise<void> {
        ...
        if (task) {
            ...
            const taskIdx = this.runningTasks.findIndex(
                (task) => task.taskId === taskExecutor.taskId,
            );
            if (taskIdx !== -1) {
