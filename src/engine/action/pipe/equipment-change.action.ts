 ...
 */
export interface EquipmentChangeActionHook
    extends ActionHook<EquipmentChangeAction, equipmentChangeActionHandler> {
 ...
 */
export type equipmentChangeActionHandler = (
    equipmentChangeAction: EquipmentChangeAction,
) => void;
...
const equipmentChangeActionPipe = (
    ...
    let matchingHooks = getActionHooks<EquipmentChangeActionHook>(
        'equipment_change',
        (equipActionHook) => {
    ...
    if (!itemDetails) {
        logger.error(
            `Item ${itemId} not registered on the server [equipment-change action pipe]`,
        );
        return null;
    }
