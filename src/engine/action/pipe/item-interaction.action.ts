 ...
 */
export interface ItemInteractionActionHook
    extends ActionHook<ItemInteractionAction, itemInteractionActionHandler> {
    // A single game item ID or a list of item IDs that this action applies to.
    itemIds?: number | number[];
    // A single UI widget ID or a list of widget IDs that this action applies to.
    widgets?:
        | { widgetId: number; containerId: number }
        | { widgetId: number; containerId: number }[];
 ...
 */
export type itemInteractionActionHandler = (
...
const itemInteractionActionPipe = (
    ...
    const playerWidget = Object.values(player.interfaceState.widgetSlots).find(
        (widget) => widget && widget.widgetId === widgetId,
    );
    ...
    let matchingHooks = getActionHooks<ItemInteractionActionHook>(
        'item_interaction',
        (plugin) => {
            ...
            if (plugin.widgets !== undefined) {
                if (Array.isArray(plugin.widgets)) {
                    ...
                    if (
                        plugin.widgets.widgetId !== widgetId ||
                        plugin.widgets.containerId !== containerId
                    ) {
                        return false;
                    }
                }
    ...
    if (!itemDetails) {
        logger.error(
            `Item ${itemId} not registered on the server [item-interaction action pipe]`,
        );
        return null;
    }
