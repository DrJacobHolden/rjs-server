 ...
 */
export interface ItemOnItemActionHook
    extends ActionHook<ItemOnItemAction, itemOnItemActionHandler> {
 ...
 */
export type itemOnItemActionHandler = (
    itemOnItemAction: ItemOnItemAction,
) => void;
...
const itemOnItemActionPipe = (
    ...
    let matchingHooks = getActionHooks<ItemOnItemActionHook>(
        'item_on_item',
        (plugin) => {
            if (questHookFilter(player, plugin)) {
                ...
                return plugin.items.some(({ item1, item2 }) => {
                    if (item2) {
                        return (
                            (item1 === used && item2 === usedWith) ||
                            (item1 === usedWith && item2 === used)
                        );
                    }
