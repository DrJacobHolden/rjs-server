 ...
 */
export interface ItemOnPlayerActionHook
    extends ActionHook<itemOnPlayerActionHandler> {
 ...
 */
export type itemOnPlayerActionHandler = (
    itemOnPlayerAction: ItemOnPlayerAction,
) => void;
...
const itemOnPlayerActionPipe = (
    ...
    let interactionActions = getActionHooks<ItemOnPlayerActionHook>(
        'item_on_player',
    ).filter(
        (plugin) =>
    ...
    const questActions = interactionActions.filter(
    ...
    if (interactionActions.length === 0) {
        player.outgoingPackets.chatboxMessage(
            `Unhandled item on player interaction: ${item.itemId} ` +
    ...
    const immediateHooks = interactionActions.filter(
        (plugin) => !plugin.walkTo,
    );

    if (walkToPlugins.length > 0) {
        player.enqueueBaseTask(
            new WalkToActorPluginTask(
                walkToPlugins,
                player,
                'otherPlayer',
                otherPlayer,
