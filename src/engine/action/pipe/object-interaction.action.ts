 ...
 */
export interface ObjectInteractionActionHook
    extends ActionHook<
 ...
 */
export type objectInteractionActionHandler = (
    objectInteractionAction: ObjectInteractionAction,
) => void;
...
const objectInteractionActionPipe = (
    ...
    let matchingHooks = getActionHooks<ObjectInteractionActionHook>(
        'object_interaction',
    ).filter(
        (plugin) =>
    ...
    if (walkToPlugins.length > 0) {
        player.enqueueBaseTask(
            new WalkToObjectPluginTask(walkToPlugins, player, landscapeObject, {
                objectConfig,
                cacheOriginal,
                option,
            }),
