 ...
 */
export interface PlayerInteractionActionHook
    extends ActionHook<
        PlayerInteractionAction,
        playerInteractionActionHandler
    > {
 ...
 */
export type playerInteractionActionHandler = (
    playerInteractionAction: PlayerInteractionAction,
) => void;
...
const playerInteractionActionPipe = (
    ...
    let matchingHooks = getActionHooks<PlayerInteractionActionHook>(
        'player_interaction',
    ).filter(
        (plugin) =>
            questHookFilter(player, plugin) &&
            stringHookFilter(plugin.options, option),
    );
    ...
    if (matchingHooks.length === 0) {
        player.sendMessage(
            `Unhandled Player interaction: ${option} @ ${position.x},${position.y},${position.level}`,
        );
        return null;
    }
