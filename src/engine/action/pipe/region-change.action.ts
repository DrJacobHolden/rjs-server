 ...
 */
export interface RegionChangeActionHook
    extends ActionHook<RegionChangeAction, regionChangeActionHandler> {
 ...
 */
export type regionChangeActionHandler = (
    regionChangeAction: RegionChangeAction,
) => void;
...
export const regionChangeActionFactory = (
    ...
    const originalMapRegionId: number =
        ((originalPosition.x >> 6) << 8) + (originalPosition.y >> 6);
    const currentMapRegionId: number =
        ((currentPosition.x >> 6) << 8) + (currentPosition.y >> 6);
...
const regionChangeActionPipe = (actionData: RegionChangeAction): void => {
    ...
    const actionList =
        getActionHooks<RegionChangeActionHook>('region_change')?.filter(
            (actionHook) => {
                ...
                if (
                    actionHook.regionTypes &&
                    actionHook.regionTypes.length !== 0
                ) {
                    let valid = false;
