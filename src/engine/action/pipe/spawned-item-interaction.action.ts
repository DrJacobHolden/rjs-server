 ...
 */
export interface SpawnedItemInteractionHook
    extends ActionHook<
 ...
 */
export type spawnedItemInteractionHandler = (
    spawnedItemInteractionAction: SpawnedItemInteractionAction,
) => void;
...
const spawnedItemInteractionPipe = (
    ...
    let matchingHooks = getActionHooks<SpawnedItemInteractionHook>(
    ...
    if (matchingHooks.length === 0) {
        player.outgoingPackets.chatboxMessage(
            `Unhandled world item interaction: ${option} ${worldItem.itemId}`,
        );
        return null;
    }
    ...
    if (!itemDetails) {
        logger.error(
            `Item ${worldItem.itemId} not registered on the server [spawned-item-interaction action pipe]`,
        );
        return null;
    }
