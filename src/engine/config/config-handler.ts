...
import {
    loadNpcSpawnConfigurations,
    NpcSpawn,
} from '@engine/config/npc-spawn-config';
import { loadShopConfigurations, Shop } from '@engine/config/shop-config';
import { Quest } from '@engine/world/actor';
import {
    ItemSpawn,
    loadItemSpawnConfigurations,
} from '@engine/config/item-spawn-config';
import {
    loadMusicRegionConfigurations,
    MusicTrack,
} from '@engine/config/music-regions-config';
import {
    loadXteaRegionFiles,
    ObjectConfig,
    XteaRegion,
...
export const widgets: {
    [key: string]: any;
} = require('../../../data/config/widgets.json');
...
export async function loadGameConfigurations(): Promise<void> {
    ...
    const { items, itemIds, itemPresets, itemGroups } =
        await loadItemConfigurations('data/config/items/');
    itemMap = items;
    itemGroupMap = itemGroups;
    itemIdMap = itemIds;
    itemPresetMap = itemPresets;

    const { npcs, npcIds, npcPresets } =
        await loadNpcConfigurations('data/config/npcs/');
    npcMap = npcs;
    npcIdMap = npcIds;
    ...
    musicRegions.forEach((song) =>
        song.regionIds.forEach((region) =>
            musicRegionMap.set(region, song.songId),
        ),
    );
...
export const findNpc = (npcKey: number | string): NpcDetails | null => {
    ...
    if (typeof npcKey === 'number') {
        ...
        if (!npcKey) {
            ...
            logger.warn(
                `NPC ${gameId} is not yet configured on the server and a matching cache NPC was not found.`,
            );
            return null;
        }
    }
    ...
    if (!npc) {
        logger.warn(
            `NPC ${npcKey} is not yet configured on the server and a matching cache NPC was not provided.`,
        );
        return null;
    }
    ...
    if (npc.extends) {
        ...
        extensions.forEach((extKey) => {
            ...
            if (extensionNpc) {
                npc = _.merge(
                    npc,
                    translateNpcServerConfig(undefined, extensionNpc),
                );
            }
        });
    }
...
export const findQuest = (questId: string): Quest | null => {
    const questKey = Object.keys(questMap).find(
        (quest) => quest.toLocaleLowerCase() === questId.toLocaleLowerCase(),
    );
...
export const findMusicTrackByButtonId = (
    buttonId: number,
): MusicTrack | null => {
    return (
        musicRegions.find((track) => track.musicTabButtonId === buttonId) ||
        null
    );
};
