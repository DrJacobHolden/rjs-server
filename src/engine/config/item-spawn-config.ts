...
export function translateItemSpawnConfig(
    config: ItemSpawnConfiguration,
): ItemSpawn {
    const spawn = new ItemSpawn(
        config.item,
...
export async function loadItemSpawnConfigurations(
    path: string,
): Promise<ItemSpawn[]> {
    const itemSpawns: ItemSpawn[] = [];

    const files = await loadConfigurationFiles<ItemSpawnConfiguration[]>(path);
    files.forEach((spawns) =>
        spawns.forEach((itemSpawn) =>
            itemSpawns.push(translateItemSpawnConfig(itemSpawn)),
        ),
    );
