...
export class NpcDetails extends NpcConfig {
    ...
    public constructor(defaultValues: { [key: string]: any }) {
        ...
        Object.keys(defaultValues).forEach(
            (key) => (this[key] = defaultValues[key]),
        );
    }
}
    ...
    return new NpcDetails({
        ...
        offensiveStats: config.offensive_stats
            ? {
                  ...
                  magicStrength:
                      config.offensive_stats.magic_strength || undefined,
                  ranged: config.offensive_stats.ranged || undefined,
                  rangedStrength:
                      config.offensive_stats.ranged_strength || undefined,
              }
            : undefined,
            ...
            if (key === 'presets') {
                ...
                if (!Number.isNaN(npcConfig.game_id)) {
                    ...
                    npcs[key] = {
                        ...
                        ...filestore.configStore.npcStore.getNpc(
                            npcConfig.game_id,
                        ),
                    };
                }
                if (npcConfig.variations) {
                    for (const variation of npcConfig.variations) {
                        try {
                            ...
                            logger.error(
                                `Error registering npc variant ${key}_${variation.suffix}`,
                            );
                            logger.error(error);
                        }
                    }
