...
export function translateNpcSpawnConfig(
    config: NpcSpawnConfiguration,
): NpcSpawn {
    return new NpcSpawn(
        config.npc,
...
export async function loadNpcSpawnConfigurations(
    path: string,
): Promise<NpcSpawn[]> {
    ...
    files.forEach((spawns) =>
        spawns.forEach((npcSpawn) =>
            npcSpawns.push(translateNpcSpawnConfig(npcSpawn)),
        ),
    );
