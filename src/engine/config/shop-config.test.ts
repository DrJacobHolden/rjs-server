...
import {
    ...
    ShopConfiguration,
    shopFactory,
} from '@engine/config/shop-config';
...
describe('shopping', () => {
    ...
    describe('shop object', () => {
        ...
        test('shop stock should be correct', () => {
            expect(
                shopConfig.stock.reduce(
                    (all, curr) => all && shop.isItemSoldHere(curr.itemKey),
                    true,
                ),
            ).toBeTruthy();
        ...
        test('player should buy item from shop at correct value', () => {
            ...
            const shopBStaffIndex = shop.container.findIndex(
                battleStaffItem.gameId,
            );
            const shopBstaff = shop.container.items[shopBStaffIndex];
            if (!shopBstaff) {
                throw new Error('Battle staff item not found in shop config');
            }
            expect(shop.getBuyFromShopPrice(battleStaffItem)).toBe(
                Math.round(
                    battleStaffItem.value * (shopConfig.shop_sell_rate || 1.0),
                ),
            );
