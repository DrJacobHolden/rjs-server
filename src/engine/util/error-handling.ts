...
export class WidgetsClosedWarning extends Error {
    constructor() {
        ...
        this.message =
            'The active widget was closed before the action could be completed.';
    }
}
...
export class ActionsCancelledWarning extends Error {
    constructor() {
        ...
        this.message =
            'Pending and active actions were cancelled before they could be completed.';
    }
}
...
export function initErrorHandling(): void {
    process.on('unhandledRejection', (error: any, promise) => {
        ...
        logger.error(
            `Unhandled promise rejection from ${promise}, reason: ${error}`,
        );
        if (error?.stack) {
