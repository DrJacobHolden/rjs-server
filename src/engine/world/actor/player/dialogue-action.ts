...
export class DialogueAction {
    ...
    public async npc(
        ...
        return this.dialogue({
            ...
            lines,
            type: 'NPC',
            npc: typeof npc === 'number' ? npc : npc.id,
        });
    }
    ...
    public async dialogue(options: DialogueOptions): Promise<DialogueAction> {
        if (
            options.lines.length < lineConstraints[options.type][0] ||
            options.lines.length > lineConstraints[options.type][1]
        ) {
        ...
        if (options.type === 'PLAYER' || options.type === 'NPC') {
            ...
            if (options.type === 'NPC') {
                ...
                this.p.outgoingPackets.updateWidgetString(
                    widgetId,
                    1,
                    cacheNpc.name || 'Unknown',
                );
            ...
            this.p.outgoingPackets.playWidgetAnimation(
                widgetId,
                0,
                options.emote,
            );
...
export const dialogueAction = async (
    player: Player,
