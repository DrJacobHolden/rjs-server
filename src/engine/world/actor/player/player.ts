...
import {
    ...
    ItemContainer,
    Item,
} from '@engine/world/items';
import { Chunk, ChunkUpdateItem } from '@engine/world/map';
import {
    PlayerCommandActionHook,
    regionChangeActionFactory,
} from '@engine/action';
...
export class Player extends Actor {
    ...
    public async init(): Promise<void> {
        ...
        const playerChunk = activeWorld.chunkManager.getChunkForWorldPosition(
            this.position,
        );
        ...
        this.skills.values.forEach((skill, index) =>
            this.outgoingPackets.updateSkill(
                index,
                this.skills.getLevel(index),
                skill.exp,
            ),
        );

        this.outgoingPackets.sendUpdateAllWidgetItems(
            widgets.inventory,
            this.inventory,
        );
        this.outgoingPackets.sendUpdateAllWidgetItems(
        ...
        for (const item of this.equipment.items) {
            if (item) {
                await this.actionPipeline.call(
                    'equipment_change',
                    this,
                    item.itemId,
                    'EQUIP',
        ...
        if (this.firstTimePlayer) {
            ...
            serverConfig.showWelcome &&
            (!serverConfig.tutorialEnabled ||
                this.savedMetadata.tutorialComplete)
        ) {
            ...
            this.outgoingPackets.updateWidgetString(
                widgets.welcomeScreen,
                16,
                'You have @yel@0 unread messages\\nin your message centre.',
            );
            this.outgoingPackets.updateWidgetString(
                widgets.welcomeScreen,
                14,
        ...
        for (const playerOption of playerOptions) {
            this.outgoingPackets.updatePlayerOption(
                playerOption.option,
                playerOption.index,
                playerOption.placement,
            );
        }
        ...
        this.inventory.containerUpdated.subscribe((event) =>
            this.inventoryUpdated(event),
        );
        this.playerEvents.on('exp', (amt) => {
            logger.info(
                `Player should have been awarded ${amt} exp if this was hooked up.`,
            );
        });
        ...
        this._lastAddress =
            (this._socket?.address() as AddressInfo)?.address || '127.0.0.1';

        if (this.rights === Rights.ADMIN) {
            this.sendCommandList(
                actionHookMap.player_command as PlayerCommandActionHook[],
            );
        }
    ...
    public logout(): void {
        ...
        if (this.quadtreeKey) {
            ...
            logger.warn(
                `Player ${this.username} has no quadtree key on logout.`,
            );
        }
    ...
    public privateMessageReceived(
        ...
        this.outgoingPackets.sendPrivateMessage(
            this.privateMessageIndex++,
            fromPlayer,
            messageBytes,
        );
    }
    ...
    public removeFriend(friendName: string): boolean {
        friendName = friendName.toLowerCase();
        const index = this.friendsList.findIndex(
    ...
    public addIgnoredPlayer(playerName: string): boolean {
        ...
        const index = this.ignoreList.findIndex(
            (ignoredPlayer) => ignoredPlayer === playerName,
        );
    ...
    public chunkChanged(chunk: Chunk): void {
        const nearbyChunks =
            activeWorld.chunkManager.getSurroundingChunks(chunk);
        if (this._nearbyChunks.length === 0) {
            this.sendChunkUpdates(nearbyChunks);
        } else {
            const newChunks = nearbyChunks.filter(
                (c1) =>
                    this._nearbyChunks.findIndex((c2) => c1.equals(c2)) === -1,
            );
        ...
        return new Promise<void>((resolve) => {
            ...
            if (this.updateFlags.mapRegionUpdateRequired) {
                if (this.position.x >= 6400) {
                    ...
                    if (this.metadata.customMap) {
                        this.outgoingPackets.constructMapRegion(
                            this.metadata.customMap,
                        );
                    } else {
                        logger.warn(
                            `Player ${this.username} is in custom map area but has no custom map set.`,
                        );
                    }
                } else {
                    this.outgoingPackets.updateCurrentMapChunk();
    ...
    public async update(): Promise<void> {
        await Promise.all([
            this.playerUpdateTask.execute(),
            this.npcUpdateTask.execute(),
        ]);
    }
    ...
    public getQuestPoints(): number {
        ...
        if (this.quests && this.quests.length !== 0) {
            this.quests
                ...
                .forEach(
                    (quest) =>
                        (questPoints += questMap[quest.questId]?.points || 0),
                );
        }
    ...
    public getQuest(questId: string): PlayerQuest {
        let playerQuest = this.quests.find(
     ...
     */
    public hasQuestRequirement(
        questId: string,
        minimumStage: QuestKey = 'complete',
    ): boolean {
        if (!questMap[questId]) {
        ...
        return (
            playerQuest.progress === minimumStage ||
            playerQuest.progress >= minimumStage
        );
    }
    ...
    public setQuestProgress(questId: string, progress: QuestKey): void {
        ...
        if (playerQuest.progress === 0 && !playerQuest.complete) {
            ...
            this.modifyWidget(widgets.questTab, {
                childId: questData.questTabId,
            ...
            this.outgoingPackets.updateClientConfig(
                widgetScripts.questPoints,
                questData.points + this.getQuestPoints(),
            );
            this.modifyWidget(widgets.questReward, {
                childId: 2,
                text: `You have completed ${questData.name}!`,
            });
            this.modifyWidget(widgets.questReward, {
                childId: 8,
            ...
            for (let i = 0; i < 5; i++) {
                if (
                    i >=
                    questData.onComplete.questCompleteWidget.rewardText.length
                ) {
                    this.modifyWidget(widgets.questReward, {
                        childId: 9 + i,
                        text: '',
                    ...
                    this.modifyWidget(widgets.questReward, {
                        childId: 9 + i,
                        text: questData.onComplete.questCompleteWidget
                            .rewardText[i],
                    });
                }
            }
            ...
            if (questData.onComplete.questCompleteWidget.itemId) {
                const cacheItemData = filestore.configStore.itemStore.getItem(
                    questData.onComplete.questCompleteWidget.itemId,
                );

                if (cacheItemData?.model2d.widgetModel) {
                    this.outgoingPackets.updateWidgetModel1(
                        widgets.questReward,
                        3,
                        cacheItemData.model2d.widgetModel,
                    );
                }
                ...
                this.outgoingPackets.updateWidgetModel1(
                    widgets.questReward,
                    3,
                    questData.onComplete.questCompleteWidget.modelId,
                );
            }
    ...
    public modifyWidget(
        ...
        if (childId !== undefined) {
            ...
            if (hidden !== undefined) {
                this.outgoingPackets.toggleWidgetVisibility(
                    widgetId,
                    childId,
                    hidden,
                );
            ...
            if (textColor !== undefined) {
                ...
                this.outgoingPackets.updateWidgetColor(
                    widgetId,
                    childId,
                    rgbTo16Bit(r, g, b),
                );
     ...
     */
    public setSidebarWidget(
        sidebarId: SidebarTab,
    ...
    public async sendMessage(
        messages: string | string[],
        options?: boolean | SendMessageOptions,
    ): Promise<boolean> {
    ...
    public teleport(newPosition: Position, updateRegion: boolean = true): void {
        ...
        const oldChunk =
            activeWorld.chunkManager.getChunkForWorldPosition(originalPosition);
        const newChunk =
            activeWorld.chunkManager.getChunkForWorldPosition(newPosition);
    ...
    public hasCoins(amount: number): number {
        return this.inventory.items.findIndex(
            (item) =>
                item !== null &&
                item.itemId === itemIds.coins &&
                item.amount >= amount,
        );
    }
    ...
    public updateCarryWeight(force: boolean = false): void {
        ...
        this._carryWeight = Math.round(
            this.inventory.weight() + this.equipment.weight(),
        );
    ...
    public settingChanged(buttonId: number): void {
        const settingsMappings = {
            ...
            1: {
                setting: 'chatEffectsEnabled',
                value: !this.settings.chatEffectsEnabled,
            },
            2: {
                setting: 'splitPrivateChatEnabled',
                value: !this.settings.splitPrivateChatEnabled,
            },
            3: {
                setting: 'twoMouseButtonsEnabled',
                value: !this.settings.twoMouseButtonsEnabled,
            },
            4: {
                setting: 'acceptAidEnabled',
                value: !this.settings.acceptAidEnabled,
            },
            // 5 is house options
     ...
     */
    public canEquipItem(item: ItemDetails | string): {
        equipable: boolean;
        ...
        missingRequirements.push(
            ...Object.entries(requirements.skills || {})
                .filter(
                    ([skill, level]) =>
                        !this.skills.hasLevel(skill as SkillName, level),
                )
                .map(
                    ([skill, level]) =>
                        `You need to be at least level ${level} ${skill} to equip this item.`,
                ),
            ...Object.entries(requirements.quests || {})
                .filter(([quest, stage]) =>
                    this.hasQuestRequirement(quest, stage),
                )
                .map(
                    ([quest]) =>
        ...
        return {
            equipable: missingRequirements.length === 0,
    ...
    public equipItem(
        ...
        const itemToEquip = getItemFromContainer(
            itemId,
            itemSlot,
            this.inventory,
        );
        ...
        if (
            !itemDetails ||
            !itemDetails.equipmentData ||
            !itemDetails.equipmentData.equipmentSlot
        ) {
            this.sendMessage(
                `Unable to equip item ${itemId}: Missing equipment data.`,
            );
            return false;
        }
        ...
        if (!equippable.equipable) {
            if (equippable.missingRequirements) {
                equippable.missingRequirements.forEach(async (s) =>
                    this.sendMessage(s),
                );
            }
            return false;
        ...
        if (itemDetails?.equipmentData) {
            ...
            if (slot === 'off_hand' && mainHandEquipped) {
                ...
                if (
                    mainHandItemData?.equipmentData &&
                    mainHandItemData.equipmentData.equipmentType ===
                        'two_handed'
                ) {
        ...
        if (itemToUnequip) {
            ...
            if (
                shouldUnequipMainHand &&
                !this.unequipItem('main_hand', false)
            ) {
                return false;
            }
    ...
    public equipmentChanged(): void {
        ...
        if (
            this.interfaceState.widgetOpen(
                'screen',
                widgets.equipmentStats.widgetId,
            )
        ) {
            this.outgoingPackets.sendUpdateAllWidgetItems(
    ...
    public getMorphedNpcDetails(originalNpc: Npc) {
        ...
        if (originalNpc.varbitId !== -1) {
            morphIndex = getVarbitMorphIndex(
                originalNpc.varbitId,
                this.metadata.configs,
            );
        } else if (originalNpc.settingId !== -1) {
            morphIndex = this.metadata.configs?.[originalNpc.settingId]
                ? this.metadata.configs[originalNpc.settingId]
                : 0;
        } else {
            logger.warn(
        ...
        if (!npcDetails) {
            logger.warn(
                `Fetched a morphed NPC, but it isn't yet registered on the server. (id-${originalNpc.id}) (morphedId-${originalNpc.childrenIds[morphIndex]})`,
            );
    ...
    public equals(player: Player): boolean {
        return (
            this.worldIndex === player.worldIndex &&
    ...
    private inventoryUpdated(event: ContainerUpdateEvent): void {
        if (event.type === 'CLEAR_ALL') {
            ...
            if (event.slot !== undefined && event.item !== undefined) {
                ...
                logger.error(
                    'Inventory update event was missing slot or item.',
                    event,
                );
            }
        }
    ...
    private sendChunkUpdates(chunks: Chunk[]): void {
        ...
        if (!instance) {
            logger.error(
                `Player ${this.username} tried to send chunk updates without an instance.`,
            );
            return;
        }
        ...
        chunks.forEach((chunk) => {
            ...
            const chunkModifications =
                instance.getInstancedChunk(
                    chunk.position.x,
                    chunk.position.y,
                    chunk.position.level,
                ) || null;
            const personalChunkModifications =
                this.personalInstance?.getInstancedChunk(
                    chunk.position.x,
                    chunk.position.y,
                    chunk.position.level,
                ) || null;
            ...
            this.findChunkUpdates(
                personalChunkModifications?.mods,
                chunkUpdateItems,
            );
    ...
    private findChunkUpdates(
        chunkMods: Map<string, TileModifications>,
        chunkUpdateItems: ChunkUpdateItem[],
    ): void {
        if (!chunkMods) {
            return;
        }
    ...
    private updateQuestTab(): void {
        ...
        Object.keys(questMap).forEach((questKey) => {
            ...
            const playerQuest = this.quests.find(
                (quest) => quest.questId === questData.id,
            );
    ...
    private updateMusicTab(): void {
        if (!this.savedMetadata.currentSongIdPlaying) {
            this.savedMetadata.currentSongIdPlaying = findSongIdByRegionId(
                activeWorld.chunkManager.getRegionIdForWorldPosition(
                    this.position,
                ),
            );
        }
        ...
        Object.keys(musicRegions).forEach((key) => {
            ...
            this.modifyWidget(widgets.musicPlayerTab, {
                childId: musicData.musicTabButtonId,
                textColor: color,
            });
        });
    }
    ...
    private addBonuses(item: Item): void {
        ...
        if (offensiveBonuses) {
            ['speed', 'stab', 'slash', 'crush', 'magic', 'ranged'].forEach(
                (bonus) =>
                    (this.bonuses.offensive[bonus] += !offensiveBonuses[bonus]
                        ? 0
                        : offensiveBonuses[bonus]),
            );
        }
        ...
        if (defensiveBonuses) {
            ['stab', 'slash', 'crush', 'magic', 'ranged'].forEach(
                (bonus) =>
                    (this.bonuses.defensive[bonus] += !defensiveBonuses[bonus]
                        ? 0
                        : defensiveBonuses[bonus]),
            );
        }

        if (skillBonuses) {
            ['strength', 'prayer'].forEach(
                (bonus) =>
                    (this.bonuses.skill[bonus] += !skillBonuses[bonus]
                        ? 0
                        : skillBonuses[bonus]),
            );
        }
    ...
    private loadSaveData(): void {
        ...
        if (!firstTimePlayer) {
            ...
            this.position = new Position(
                playerSave.position.x,
                playerSave.position.y,
                playerSave.position.level,
            );
            ...
            this._lastAddress =
                playerSave.lastLogin?.address ||
                (this._socket?.address() as AddressInfo)?.address ||
                '127.0.0.1';
        } else {
