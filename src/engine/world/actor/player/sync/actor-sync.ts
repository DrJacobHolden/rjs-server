...
export function registerNewActors(
    ...
    let newActors: QuadtreeKey[] = nearbyActors.filter(
        (m1) => !trackedActors.includes(m1.actor),
    );
    if (newActors.length > 50) {
        ...
        newActors = newActors
            .sort(
                (a, b) =>
                    player.position.distanceBetween(a.actor.position) -
                    player.position.distanceBetween(b.actor.position),
            )
            .slice(0, 50);
    }
...
export function syncTrackedActors(
    ...
    for (let i = 0; i < trackedActors.length; i++) {
        ...
        if (
            exists &&
            nearbyActors.findIndex((m) => m.actor.equals(trackedActor)) !==
                -1 &&
