...
export class NpcSyncTask extends SyncTask<void> {
    ...
    public async execute(): Promise<void> {
        return new Promise<void>((resolve) => {
            const npcUpdatePacket: Packet = new Packet(
                128,
            ...
            const nearbyNpcs = activeWorld.npcTree
                ...
                .filter((collision) => {
                    ...
                    return (
                        npc?.initialized &&
                        npc.instanceId === this.player.instance.instanceId
                    );
                });
            ...
            this.player.trackedNpcs = syncTrackedActors(
                ...
                (actor) =>
                    this.appendUpdateMaskData(actor as Npc, updateMaskData),
            ...
            registerNewActors(
                ...
                (actor) => {
                    ...
                    const positionOffsetX =
                        newNpc.position.x - this.player.position.x;
                    const positionOffsetY =
                        newNpc.position.y - this.player.position.y;
                    ...
                    npcUpdatePacket.putBits(
                        1,
                        newNpc.updateFlags.updateBlockRequired ? 1 : 0,
                    ); // Update is required
