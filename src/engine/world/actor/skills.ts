...
export class Skills extends SkillShortcuts {
    ...
    private static MAXIMUM_INDEX: number =
        Skills.EXPERIENCE_LOOKUP_TABLE.length - 1;
    ...
    public constructor(
        ...
        Object.keys(Skill)
            ...
            .forEach(
                (skillName) =>
                    (this[skillName] = new SkillShortcut(
                        this,
    ...
    public getTotalLevel(): number {
        return this._values
            .map((skillValue) => skillValue.level)
    ...
    public getCombatLevel(): number {
        const combatLevel =
            (this.defence.level +
                ...
                Math.floor(this.prayer.level / 2)) *
            0.25;
    ...
    public getLevel(
        ...
        ignoreLevelModifications: boolean = false,
    ): number {
        const s = this.get(skill);
        return s.modifiedLevel !== undefined && !ignoreLevelModifications
            ? s.modifiedLevel
            : s.level;
    }
    ...
    public getLevelForExp(
        exp: number,
        index: number | undefined = undefined,
    ): number {
        const start = Skills.confine(
            index || Skills.MAXIMUM_INDEX,
            Skills.MINIMUM_LEVEL,
            Skills.MAXIMUM_INDEX,
        );
    ...
    public addExp(skill: number | SkillName, exp: number): void {
        ...
        if (this.actor instanceof Player) {
            this.actor.outgoingPackets.updateSkill(
                this.getSkillId(skill),
                finalLevel,
                finalExp,
            );
        }
    ...
    public showLevelUpDialogue(skill: number | SkillName, level: number): void {
        ...
        player.modifyWidget(widgetId, {
            childId: 0,
            text:
                `<col=000080>Congratulations, you just advanced ${startsWithVowel(skillName) ? 'an' : 'a'} ` +
                `${skillName} level.</col>`,
        });
        player.modifyWidget(widgetId, {
            childId: 1,
