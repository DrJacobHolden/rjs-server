...
export class CollisionMap {
    ...
    public constructor(
        ...
        y: number,
        heightLevel: number,
        options?: { chunk?: Chunk; instance?: WorldInstance },
    ) {
    ...
    public markGameObject(
        ...
        const objectDetails = filestore.configStore.objectStore.getObject(
            landscapeObject.objectId,
        );

        if (!objectDetails) {
            logger.error(
                `Could not find object details for object id: ${landscapeObject.objectId} when marking collision map.`,
            );
            return;
        }
    ...
    public getSiblingCollisionMap(
        ...
        if (this.instance) {
            const instanceChunk = this.instance.getInstancedChunk(
                this.x + offsetX,
                this.y + offsetY,
                this.heightLevel,
            );
