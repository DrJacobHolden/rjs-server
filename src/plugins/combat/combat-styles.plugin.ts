...
} from '@engine/action/pipe/equipment-change.action';
import {
    ItemDetails,
    WeaponStyle,
    weaponWidgetIds,
...
export function updateCombatStyle(
    player: Player,
...
export function showUnarmed(player: Player): void {
    player.modifyWidget(widgets.defaultCombatStyle, {
        childId: 0,
        text: 'Unarmed',
    });
...
export function setWeaponWidget(
    player: Player,
    weaponStyle: WeaponStyle,
    itemDetails: ItemDetails | null,
): void {
    player.modifyWidget(weaponWidgetIds[weaponStyle], {
        childId: 0,
        text: itemDetails?.name || 'Unknown',
    });
    ...
    if (player.savedMetadata.combatStyle) {
        updateCombatStyle(
            player,
            weaponStyle,
            player.savedMetadata.combatStyle[1] || 0,
        );
    }
}
...
export function updateCombatStyleWidget(player: Player): void {
    ...
    if (equippedItem) {
        ...
        const weaponStyle =
            itemDetails?.equipmentData?.weaponInfo?.style || null;
...
const equip: equipmentChangeActionHandler = ({
    player,
    itemDetails,
    equipmentSlot,
}) => {
...
const combatStyleSelection: buttonActionHandler = ({ player, buttonId }) => {
    ...
    if (equippedItem) {
        weaponStyle =
            findItem(equippedItem.itemId)?.equipmentData?.weaponInfo?.style ||
            null;
    ...
    const combatStyle = combatStyles[weaponStyle].findIndex(
        (combatStyle) => combatStyle.button_id === buttonId,
    );
    if (combatStyle !== -1) {
