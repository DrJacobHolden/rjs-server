...
const debugMapRegion = (
    ...
    player.sendMessage(
        `Region ${key} - ${activeWorld.chunkManager.getRegionIdForWorldPosition(player.position)}`,
    );
    ...
    for (let y = 63; y >= 0; y--) {
        ...
        for (let x = 0; x < 64; x++) {
            ...
            if (
                tileWorldX === player.position.x &&
                tileWorldY === player.position.y
            ) {
                line[x] = '@';
            } else if (region.mapFile?.tileSettings) {
                const tileSettings = activeWorld.chunkManager.getTile(
                    new Position(tileWorldX, tileWorldY, level),
                ).settings;
...
const tileDebugHandler: commandActionHandler = ({ player }) => {
    ...
    const tile0 = activeWorld.chunkManager.getTile(
        player.position.copy().setLevel(0),
    );
    const tile1 = activeWorld.chunkManager.getTile(
        player.position.copy().setLevel(1),
    );
    const tile2 = activeWorld.chunkManager.getTile(
        player.position.copy().setLevel(2),
    );
    const tile3 = activeWorld.chunkManager.getTile(
        player.position.copy().setLevel(3),
    );
