...
const handler: commandActionHandler = ({ player, args }) => {
    ...
    if (playerCount > World.MAX_PLAYERS - 1) {
        player.sendMessage(
            `Error: Max player count is ${World.MAX_PLAYERS - 1}.`,
        );
        return;
    }
    ...
    const spawnChunk = activeWorld.chunkManager.getChunkForWorldPosition(
        new Position(x, y, 0),
    );
    ...
    const playerSpawnCount =
        playerCount > worldSlotsRemaining ? worldSlotsRemaining : playerCount;

    if (playerSpawnCount < playerCount) {
        player.sendMessage(
            `Warning: There was only room for ${playerSpawnCount}/${playerCount} player spawns.`,
        );
    }
