...
export const action: itemInteractionActionHandler = (details) => {
    ...
    if (!player.skills.hasLevel('herblore', herb.level)) {
        player.sendMessage(
            `You need a Herblore level of ${herb.level} to identify this herb.`,
            true,
        );
        return;
    }
    ...
    if (!inventoryItem) {
        logger.warn(
            `[herblore] Player ${player.username} tried to clean herb without having it in their inventory.`,
        );
        return;
    }
    ...
    if (inventoryItem.itemId !== herb.grimy.gameId) {
        logger.warn(
            `[herblore] Player ${player.username} tried to clean herb but itemId did not match.`,
        );
        return;
    }
