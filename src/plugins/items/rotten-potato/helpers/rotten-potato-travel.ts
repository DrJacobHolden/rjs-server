...
const pageCount =
    INTRO_PAGE_COUNT +
    Math.round(activeWorld.travelLocations.locations.length / ITEMS_PER_PAGE);
...
export function openTravel(player: Player, page: number) {
    ...
    player.modifyWidget(widget.widgetId, {
        childId: 98,
        text:
            widget.metadata.page * 2 - 1 === 1
                ? ''
                : `Page ${widget.metadata.page * 2 - 1} `,
    });
    ...
    let currentLocation =
        ITEMS_PER_PAGE * 2 * (page - INTRO_PAGE_COUNT - 1) +
        INTRO_PAGE_COUNT * ITEMS_PER_PAGE;

    const historyPage = [
        '<col=CCCCFF>Last locations</col>',
        '',
        '',
        ...(player.savedMetadata.lastTravel || new Array(10)).map(
            (location: number | undefined) =>
                location === undefined
                    ? ''
                    : activeWorld.travelLocations.locations[location]?.name,
        ),
    ];
    ...
    if (widget.metadata.page * 2 - 1 === 1) {
        ...
        for (let i = 0; i < ITEMS_PER_PAGE * 2; i += 2) {
            player.modifyWidget(widget.widgetId, {
                childId: 101 + i,
                text:
                    activeWorld.travelLocations.locations[currentLocation]
                        ?.name || '',
                hidden:
                    !activeWorld.travelLocations.locations[currentLocation]
                        ?.name,
    ...
    for (let i = 0; i < 30; i += 2) {
        player.modifyWidget(widget.widgetId, {
            childId: 131 + i - 1,
            hidden:
                !activeWorld.travelLocations.locations[currentLocation]?.name,
        });
        player.modifyWidget(widget.widgetId, {
            childId: 131 + i,
            text:
...
export const travelMenuInteract: widgetInteractionActionHandler = (details) => {
    ...
    if (selectedIndex !== undefined) {
        let teleportIndex =
            selectedIndex + 30 * (playerWidget.metadata.page - 1);
        ...
        if (teleportIndex < INTRO_PAGE_COUNT * ITEMS_PER_PAGE) {
            ...
            teleportIndex =
                details.player.savedMetadata.lastTravel[teleportIndex - 3];
            if (!teleportIndex) {
                openTravel(details.player, playerWidget.metadata.page);
            }
        } else {
        ...
        const newTravelLog = [
            teleportIndex,
            ...details.player.savedMetadata.lastTravel.slice(0, 9),
        ];
        ...
        details.player.teleport(
            activeWorld.travelLocations.locations[teleportIndex].position,
        );
