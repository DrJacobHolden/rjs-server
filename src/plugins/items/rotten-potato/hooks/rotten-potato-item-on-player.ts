...
export const potatoOnPlayer: itemOnPlayerActionHandler = (details) => {
    const widget = details.player.interfaceState.openWidget(
        widgets.bank.depositBoxWidget.widgetId,
        {
            slot: 'screen',
            fakeWidget: 3100001,
        },
    ...
    details.player.outgoingPackets.sendUpdateAllWidgetItems(
        widgets.bank.depositBoxWidget,
        details.otherPlayer.inventory,
    );
...
export const potatoManipulatePlayerInventory: itemInteractionActionHandler = (
    details,
) => {
    const playerWidget = details.player.interfaceState.findWidget(
        widgets.bank.depositBoxWidget.widgetId,
    );
    ...
    if (
        !details.player.inventory.canFit({
            itemId: itemIdToAdd,
    ...
    while (countToRemove > 0 && otherPlayer.inventory.has(details.itemId)) {
        ...
        if (!invItem) {
            logger.error(
                `Could not find item ${details.itemId} in inventory at slot ${invIndex} in rotten potato`,
            );
            return;
        }
    ...
    otherPlayer.outgoingPackets.sendUpdateAllWidgetItems(
        widgets.inventory,
        otherPlayer.inventory,
    );
};
