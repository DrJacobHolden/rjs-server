...
function removeCoins(
    ...
    if (!coins) {
        logger.error(
            `Could not find coins in inventory at slot ${coinsIndex} while trying to remove coins`,
        );
        return;
    }

    const amountAfterPurchase = coins.amount - cost;
    inventory.set(coinsIndex, {
        itemId: itemIds.coins,
        amount: amountAfterPurchase,
    });
}
...
export const handler: itemInteractionActionHandler = (details) => {
    ...
    if (!buyItem) {
        logger.error(
            `Could not find cache item for item id ${itemId} in shop ${openedShopKey}`,
        );
        return;
    }
    ...
    if (buyItem.stackable) {
        const inventoryStackSlot = inventory.items.findIndex(
        ...
        if (inventoryStackSlot === -1) {
            ...
            shopContainer.set(itemSlot, {
                itemId,
                amount: shopItem.amount - buyAmount,
            });
