...
export const handler: itemInteractionActionHandler = (details) => {
    ...
    const shopItemIndex = shopContainer.items.findIndex(
    ...
    if (itemDetails.stackable) {
        if (inventoryItem.amount < sellAmount) {
            ...
            inventory.set(itemSlot, {
                itemId,
                amount: inventoryItem.amount - sellAmount,
            });
        }
    } else {
        const foundItems = inventory.items
            .map((item, i) =>
                item !== null && item.itemId === itemId ? i : null,
            )
            .filter((i) => i !== null);
        ...
        for (let i = 0; i < sellAmount; i++) {
            ...
            if (!item) {
                throw new Error(
                    `Inventory item was not present, for item id ${itemId} in inventory, while trying to sell`,
                );
            }
    ...
    if (!shopItem) {
        shopContainer.set(shopContainer.getFirstOpenSlot(), {
            itemId,
            amount: sellAmount,
        });
    } else {
