...
musicRegions.forEach((song) =>
    song.regionIds.forEach((region) => musicRegionMap.set(region, song.songId)),
);
...
const regionChangedHandler = ({ player, currentMapRegionId }): void => {
    ...
    if (!player.musicTracks.includes(songId)) {
        ...
        player.sendMessage(
            `You have unlocked a new music track: <col=ef101f>${songName}.</col>`,
        );
        player.modifyWidget(widgets.musicPlayerTab, {
            childId: musicTrack.musicTabButtonId,
...
const playerInitHandler: playerInitActionHandler = ({ player }): void => {
    ...
    regionChangedHandler({
        player,
        currentMapRegionId:
            ((player.position.x >> 6) << 8) + (player.position.y >> 6),
    });
};
