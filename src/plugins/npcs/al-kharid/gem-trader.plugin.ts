...
const talkToAction: npcInteractionActionHandler = (details) => {
    ...
    dialogueAction(player)
        .then(async (d) =>
            d.npc(npc, DialogueEmote.CALM_TALK_1, [
                'Good day to you, traveller.',
                'Would you be interested in buying some gems?',
            ]),
        )
        .then(async (d) =>
            d.options('Would you be interested in buying some gems?', [
                'Yes, please.',
                'No, thank you.',
            ]),
        )
        .then(async (d) => {
            switch (d.action) {
                case 1:
                    return d
                        .player(DialogueEmote.JOYFUL, ['Yes, please!'])
                        .then((d) => {
                            tradeAction(details);
                            return d;
                        });
                case 2:
                    return d
                        ...
                        .then(async (d) =>
                            d.npc(npc, DialogueEmote.ANNOYED, [
                                'Eh, suit yourself.',
                            ]),
                        )
                        .then((d) => {
