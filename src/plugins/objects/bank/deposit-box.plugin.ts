...
export const openDepositBoxInterface: objectInteractionActionHandler = ({
    player,
}) => {
...
export const depositItem: itemInteractionActionHandler = (details) => {
    ...
    while (countToRemove > 0 && playerInventory.has(details.itemId)) {
        ...
        if (!invItem) {
            throw new Error(
                `Inventory item was not present, for item id ${details.itemId} in bank, while trying to deposit`,
            );
        }
