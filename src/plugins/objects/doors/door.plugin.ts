...
export const action: objectInteractionActionHandler = ({
    player,
    object: door,
    position,
    cacheOriginal,
    ...
    if (!doorConfig) {
        ...
        opening = false;
        hingeConfig =
            doorConfig.hinge === 'LEFT' ? rightHingeDir : leftHingeDir;
        replacementDoorId = doorConfig.closed;
    } else {
        hingeConfig =
            doorConfig.hinge === 'LEFT' ? leftHingeDir : rightHingeDir;
        replacementDoorId = doorConfig.open;
    }

    const startDir = WNES[door.orientation];
    const endDir = hingeConfig[startDir];
    const endPosition = position.step(
        opening ? 1 : -1,
        opening ? startDir : endDir,
    );
    ...
    hooks: [
        {
            ...
            objectIds: [
                1530, 4465, 4467, 3014, 3017, 3018, 3019, 1536, 1537, 1533,
                1531, 1534, 12348, 11993, 11994, 13001, 13002,
            ],
            options: ['open', 'close'],
            walkTo: true,
            handler: action,
