...
export const handler: playerInitActionHandler = ({ player }) => {
    ...
    player.outgoingPackets.updateClientConfig(
        widgetScripts.mouseButtons,
        settings.twoMouseButtonsEnabled ? 0 : 1,
    );
    player.outgoingPackets.updateClientConfig(
        widgetScripts.splitPrivateChat,
        settings.splitPrivateChatEnabled ? 1 : 0,
    );
    player.outgoingPackets.updateClientConfig(
        widgetScripts.chatEffects,
        settings.chatEffectsEnabled ? 0 : 1,
    );
    player.outgoingPackets.updateClientConfig(
        widgetScripts.acceptAid,
        settings.acceptAidEnabled ? 1 : 0,
    );
    player.outgoingPackets.updateClientConfig(
        widgetScripts.musicVolume,
        settings.musicVolume,
    );
    player.outgoingPackets.updateClientConfig(
        widgetScripts.soundEffectVolume,
        settings.soundEffectVolume,
    );
    player.outgoingPackets.updateClientConfig(
        widgetScripts.areaEffectVolume,
        settings.areaEffectVolume,
    );
    player.outgoingPackets.updateClientConfig(
        widgetScripts.runMode,
        settings.runEnabled ? 1 : 0,
    );
    player.outgoingPackets.updateClientConfig(
        widgetScripts.autoRetaliate,
        settings.autoRetaliateEnabled ? 0 : 1,
    );
    player.outgoingPackets.updateClientConfig(
    ...
    player.outgoingPackets.updateClientConfig(
        ...
        settings.bankInsertMode,
    );
    player.outgoingPackets.updateClientConfig(
        widgetScripts.bankWithdrawNoteMode,
        settings.bankWithdrawNoteMode,
    );
