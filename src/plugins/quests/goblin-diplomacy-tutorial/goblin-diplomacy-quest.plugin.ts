...
export function unlockAvailableTabs(
    player: Player,
    availableTabs?: number,
): void {
    ...
    for (let i = 0; i < availableTabs; i++) {
        ...
        player.setSidebarWidget(
            tutorialTabWidgetOrder[i][0],
            tutorialTabWidgetOrder[i][1],
        );
    }
...
export function npcHint(player: Player, npcKey: string | number): void {
    if (typeof npcKey === 'string') {
        ...
        if (!npc) {
            logger.warn(
                `Can not provide NPC hint; NPC ${npcKey} is not yet registered on the server.`,
            );
            return;
        }
    ...
    defaultPlayerTabWidgets().forEach((widgetId: number, tabIndex: number) => {
        if (widgetId !== -1) {
            player.outgoingPackets.sendTabWidget(
                tabIndex,
                widgetId === widgets.logoutTab ? widgetId : null,
            );
        }
    });
...
export async function spawnGoblinBoi(
    player: Player,
    spawnPoint: 'beginning' | 'end',
): Promise<void> {
    const nearbyGoblins = activeWorld.findNpcsByKey(
        'rs:goblin',
        player.instance.instanceId,
    );
    ...
    if (spawnPoint === 'beginning') {
        ...
        const goblin = await activeWorld.spawnNpc(
            'rs:goblin',
            new Position(3221, 3257),
            'SOUTH',
            0,
            player.instance.instanceId,
        );
        ...
        await activeWorld.spawnNpc(
            'rs:goblin',
            new Position(3219, 3246),
            'SOUTH',
            0,
            player.instance.instanceId,
        );
...
const trainingSwordEquipAction: equipmentChangeActionHandler = async ({
    player,
    itemDetails,
}) => {
    const progress = player.getQuest('tyn:goblin_diplomacy').progress;
    ...
    if (progress === 85) {
        ...
        if (
            (itemDetails.key === 'rs:training_sword' && shieldEquipped) ||
...
export default {
    ...
    hooks: [
        ...
        },
        {
            ...
            handler: questDialogueActionFactory(
                QUEST_ID,
                runescapeGuideDialogueHandler,
                tutorialHandler,
            ),
