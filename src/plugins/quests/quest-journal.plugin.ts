...
export const handler: buttonActionHandler = async ({ player, buttonId }) => {
    const quest = Object.values<Quest>(questMap).find(
        (quest) => quest.questTabId === buttonId,
    );
    if (!quest) {
        return;
    }
    ...
    player.modifyWidget(widgets.questJournal, {
        childId: 2,
        text: `@dre@${quest.name}`,
    });
