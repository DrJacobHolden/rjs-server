    ...
    50: (player) => {
        ...
        const questLogIngredientData = [
            ...
            },
            {
                ...
                haveText:
                    '<str><col=000000>I have a piece of burnt meat with me,</str>',
        ...
        <col=000080>raw beef.`,
            },
            {
                ...
                dontHaveText:
                    '<col=800000>An onion<col=000080>, I could probably find one at a farm.',
            },
            {
                itemId: itemIds.witchesPotion.eyeOfNewt,
                haveText:
                    '<str><col=000000>I have an eye of newt with me,</str>',
...
const attemptToDrinkBeforeAllowed: objectInteractionActionHandler = async (
    details,
) => {
    const { player, object } = details;
    player.face(new Position(object.x, object.y));
...
const dialogueIngredientQuestions: npcInteractionActionHandler = (details) => {
    ...
    const hasRatsTail = player.hasItemInInventory(
        itemIds.witchesPotion.ratsTail,
    );
    const hasBurntMeat = player.hasItemInInventory(
        itemIds.witchesPotion.burntMeat,
    );
    const hasOnion = player.hasItemInInventory(itemIds.witchesPotion.onion);
    const hasEyeOfNewt = player.hasItemInInventory(
        itemIds.witchesPotion.eyeOfNewt,
    );
    ...
    dialogue(
        ...
        [
            ...
            (subtree, tag_NO_INGREDIENTS) => [
                (player) => [
                    Emote.HAPPY,
                    `I've been looking for those ingredients.`,
                ],
                (hetty) => [Emote.HAPPY, 'So what have you found so far?'],
                (player) => [
                    Emote.GENERIC,
                    `I'm afraid I don't have any of them yet.`,
                ],
                (hetty) => [
                    Emote.SAD,
...
const afterQuestDialogue: npcInteractionActionHandler = (details) => {
    ...
    dialogue(
        ...
        [
            ...
            (player) => [
                Emote.HAPPY,
                `I'm practicing and slowly getting better.`,
            ],
