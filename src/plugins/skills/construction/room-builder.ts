...
export const canBuildNewRoom = (player: Player): Coords | null => {
    ...
    if (
        existingRoom &&
        existingRoom.type !== 'empty_grass' &&
        existingRoom.type !== 'empty'
    ) {
...
export const roomBuilderWidgetHandler: buttonActionHandler = async ({
    player,
    buttonId,
}) => {
    ...
    playerCustomMap.chunks[newRoomCoords.level][newRoomCoords.x][
        newRoomCoords.y
    ] = createdRoom;
    ...
    await dialogue(
        ...
        [
            (options, tag_Home) => [
                ...
                [
                    execute(() => {
                        createdRoom.orientation =
                            createdRoom.orientation > 0
                                ? createdRoom.orientation - 1
                                : 3;
                        openHouse(player);
                    }),
                    goto('tag_Home'),
...
export const doorHotspotHandler: objectInteractionActionHandler = ({
    player,
}) => {
