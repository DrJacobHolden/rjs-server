...
const tinderboxOnLogHandler: itemOnItemActionHandler = (details) => {
    ...
    if (player.skills.firemaking.level < skillInfo.requiredLevel) {
        player.sendMessage(
            `You need a Firemaking level of ${skillInfo.requiredLevel} to light this log.`,
        );
        return;
    }
    ...
    const worldItemLog = player.instance.spawnWorldItem(log, player.position, {
        owner: player,
        expires: 300,
    });
