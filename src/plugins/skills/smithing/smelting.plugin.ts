...
export const openSmeltingInterface: objectInteractionActionHandler = (
    details,
) => {
...
const loadSmeltingInterface = (details: ObjectInteractionAction) => {
    const theKnightsSwordQuest = details.player.quests.find(
        (quest) => quest.questId === 'theKnightsSword',
    );

    // Send the items to the widget.
    widgetItems.forEach((item) => {
        details.player.outgoingPackets.setItemOnWidget(
            widgets.furnace.widgetId,
            item.slot.modelId,
            item.bar.barId,
            125,
        );
        if (
            !details.player.skills.hasLevel(
                Skill.SMITHING,
                item.bar.requiredLevel,
            )
        ) {
            details.player.modifyWidget(widgets.furnace.widgetId, {
                childId: item.slot.titleId,
        ...
        if (
            Boolean(item.bar.quest) &&
            (!theKnightsSwordQuest || theKnightsSwordQuest.complete)
        ) {
            details.player.modifyWidget(widgets.furnace.widgetId, {
                childId: item.slot.titleId,
                textColor: colors.red,
...
export const buttonClicked: buttonActionHandler = (details) => {
    ...
    if (!smeltable) {
        details.player.sendMessage(
            `Could not find smeltable for button id ${details.buttonId}. Please tell a dev.`,
        );
        return;
    }
    ...
    const numericInputSpinSubscription =
        details.player.numericInputEvent.subscribe((number) => {
    ...
    const actionCancelledSpinSubscription =
        details.player.actionsCancelled.subscribe(() => {
            actionCancelledSpinSubscription?.unsubscribe();
            numericInputSpinSubscription?.unsubscribe();
        });
